...
def plot_ecg(ts=None,
    ...
    ax2.plot(ts, filtered, linewidth=MAJOR_LW, label='Filtered')
    R1, S1, Q1=QRS_test.EKG_QRS_detect(filtered, 360, True, False)
  
    # adding the R-Peaks
    ax2.scatter(
        ts[rpeaks], filtered[rpeaks], color="m", linewidth=MINOR_LW, label="R-peaks"
    )
    ax2.scatter(
        ts[S1], filtered[S1], color="m", linewidth=MINOR_LW, label="S-peaks"
    )
    ax2.scatter(
        ts[Q1], filtered[Q1], color="m", linewidth=MINOR_LW, label="Q-peaks"
    )
